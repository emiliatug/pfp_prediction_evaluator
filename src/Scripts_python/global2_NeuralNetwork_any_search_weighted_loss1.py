...
def mainprocess(
    ...
    for model_name in model_list:
        ...
        list_optimizers_fx = [Adam, Adagrad, RMSprop]
        list_optimizers_fx_names = ["Adam", "Adagrad", "RMSprop"]
        ...
        if org_var[4] == "epoch_opt_no_early_stop":
            fold_list = []
            fold_list.append([int(org_var[5]), int(org_var[6]), int(org_var[7]), int(org_var[8]), int(org_var[9])])
            fold_list = fold_list[0]
            # fold_list=dict_cv_data_cpu[model_name].keys()
        else:
            fold_list = dict_cv_data_cpu[model_name].keys()
        # for fold in dict_cv_data_cpu[model_name].keys():
        # if fold==3 or fold==2:
        print("This is the final fold list", fold_list)
        for fold in fold_list:
            print("This is an ind fold", fold)
            fold = int(fold)
            # print(type(fold),fold, model_name)
            # print(dict_cv_data_cpu.keys())
            # print(dict_cv_data_cpu[model_name].keys())
            # print("one",dict_cv_data_cpu[model_name][0].keys())
            # print("one",dict_cv_data_cpu[model_name][fold].keys())
            ...
            for n_hidden_layers in input15:
                ...
                for opt_name, opt in optimizers.items():
                    ...
                    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
                        filepath=f"{output_dir_files_models}/NN_model_manually_tunned_{org_var[3]}_fold{fold}_hidden_layers{n_hidden_layers}_{opt_name}.h5",
                    ...
                    history = model.fit(
                        ...
                        epochs=100,
                        batch_size=256,
                        callbacks=[*([early_stopping] if org_var[4] != "epoch_opt_no_early_stop" else []),
                                   lr_scheduler,
                    ...
                    # if org_var[4] != "epoch_opt_no_early_stop":
                    #     model.save(f"{output_dir_files_models}/NN_model_manually_tunned_{org_var[3]}_fold{fold}_hidden_layers{n_hidden_layers}_{opt_name}.keras")
